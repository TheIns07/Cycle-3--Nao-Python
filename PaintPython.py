...
class PaintGUI:
    def __init__(self):
        self.root = Tk()
        self.root.title("Aplicación de Dibujo en Python")
        ...
        self.colorPicker = Button(self.btn_frame, text="Change Color", command=self.change_color)
        self.colorPicker.grid(row=1, column=1, sticky=W+E)
    ...
    def save(self):
        filename = filedialog.asksaveasfilename(initialfile="untitled.png",
                                                defaultextension="png", filetypes=[("PNG", "JPEG"), (".png",".jpeg")])
    ...
    def on_closing(self):
        answer = messagebox.askyesnocancel("Salir", "¿Quieres salir sin guardar?", parent = self.root)
